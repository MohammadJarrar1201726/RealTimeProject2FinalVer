...
int main(int argc, char **argv)
{
    ...
    while (1)
    {
        ...
        for (int i = 0; i < plane.number_of_containers; i++)
        {
            ...
            fflush(stdout);
            // send information to the opengl --> (c) for container information
            ...
            // time to drop container (height)
            // occupation (SNIPER)
            int time_drop_contener = 5, j;
            for (j = 0; j < time_drop_contener; j++)
            {
                sleep(1);
                ...
                if (attack > 90)
                {
                    if (j < 2)
                    {
                        printf("[Attack]: Shooting All Plane %d's Container of(%dKg) Amount", plane.my_id, plane.containers[i].flour_amount_kg);
                        fflush(stdout);
                    ...
                    {
                        printf("[Attack]: Attack Plane %d's Container of(%dKg) Amount --> (%dKg)", plane.my_id, plane.containers[i].flour_amount_kg, plane.containers[i].flour_amount_kg - 200);
                        fflush(stdout);
        ...
        fflush(stdout);
        // for refill send message and find the new numberOfcontainers;
